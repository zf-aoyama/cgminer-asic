...
static char *avalon_set_device(struct cgpu_info *avalon, char *option, char *setting, char *replybuf, size_t siz)
{
	...
	if (strcasecmp(option, "help") == 0) {
		snprintf(sizebuf, siz, "freq: range %d-%d millivolts: range %d-%d",
					AVALON_MIN_FREQUENCY, AVALON_MAX_FREQUENCY,
	...
	if (strcasecmp(option, "millivolts") == 0 || strcasecmp(option, "mv") == 0) {
		if (!is_bitburner(avalon)) {
			snprintf(sizebuf, siz, "%s cannot set millivolts", avalon->drv->name);
			return replybuf;
		}

		if (!setting || !*setting) {
			snprintf(sizebuf, siz, "missing millivolts setting");
			return replybuf;
		}

		val = atoi(setting);
		if (val < BITBURNER_MIN_COREMV || val > BITBURNER_MAX_COREMV) {
			snprintf(sizebuf, siz, "invalid millivolts: '%s' valid range %d-%d",
		...
		if (bitburner_set_core_voltage(avalon, val))
			...
			snprintf(sizebuf, siz, "Set millivolts failed");
			return replybuf;
		}
	}
	...
	if (strcasecmp(option, "freq") == 0) {
		if (!setting || !*setting) {
			snprintf(sizebuf, siz, "missing freq setting");
			return replybuf;
		}

		val = atoi(setting);
		if (val < AVALON_MIN_FREQUENCY || val > AVALON_MAX_FREQUENCY) {
			snprintf(sizebuf, siz, "invalid freq: '%s' valid range %d-%d",
	...
	snprintf(sizebuf, siz, "Unknown option: %s", option);
	return replybuf;
}
