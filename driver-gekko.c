	...
	while (info->mining_state != MINER_SHUTDOWN) {
		...
		if (info->chips == 0 || compac->deven == DEV_DISABLED || compac->usbinfo.nodev || info->mining_state != MINER_MINING) {
			...
			if (low_eff && ms_tdiff(&now, &info->last_frequency_adjust) > MS_MINUTE_10 && ms_tdiff(&now, &info->last_pool_lost) > MS_MINUTE_10) {
				...
				{
					struct ASIC_INFO *asic = &info->asics[i];
					asic->frequency_requested = new_frequency;
				}
				cgtime(&info->last_frequency_adjust);
				cgtime(&info->monitor_time);
			}
...
static void *compac_handle_rx(void *object, int read_bytes)
{
	...
	if (cmd_resp && info->rx[0] == 0x80) {
		...
		if (info->asic_type == BM1387 && (info->rx[2] == 0 || (info->rx[3] >> 4) == 0 || (info->rx[3] & 0x0f) == 0)) {
			...
			applog(LOG_WARNING,"%d: %s %d - bad frequency", compac->cgminer_id, compac->drv->name, compac->device_id);
		} else {
			if (info->asic_type == BM1387) {
...
*/
	...
	if ((*init_count) != 0 && (*init_count) % 5 == 0) {
		if ((*init_count) >= 15) {
			...
			applog(LOG_WARNING, "%d: %s %d - forcing usb_nodev()", compac->cgminer_id, compac->drv->name, compac->device_id);
			usb_nodev(compac);
		}
...
static void compac_statline(char *buf, size_t bufsiz, struct cgpu_info *compac)
{
	...
	if (info->asic_type == BM1387) {
		if (info->micro_found) {
			...
			if (opt_log_output) {
				sprintf(asic_statline, "BM1387:%02d%-1s %.2fMHz %s%-13s", info->chips, ab, info->frequency, asic_stat, ms_stat);
			} else {
				sprintf(asic_statline, "BM1387:%02d%-1s %.2fMHz %s%s", info->chips, ab, info->frequency, asic_stat, eff_stat);
			}
		}
	} else {
		if (opt_log_output) {
			sprintf(asic_statline, "BM1384:%02d  %.2fMHz %s%-13s", info->chips, info->frequency, asic_stat, ms_stat);
		} else {
...
static void compac_shutdown(struct thr_info *thr)
{
	struct cgpu_info *compac = thr->cgpu;
	struct COMPAC_INFO *info = compac->device_data;
	applog(LOG_WARNING, "%d: %s %d - shutting down", compac->cgminer_id, compac->drv->name, compac->device_id);
	if (!compac->usbinfo.nodev) {
		if (info->asic_type == BM1387) {
